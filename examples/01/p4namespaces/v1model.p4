...
// Note that with the namespaces approach, there is NO NEED for this
// idiom that is often used for include files, because p4c is
// guaranteed to import a namespace at most once.
...
// These are the few names from the core namespace that are currently
// used within the definitions of the v1model architecture.  We could
// also potentially use `#include <core.p4>` intended for use by P4
// developers in their programs, which would import all names from the
// core namespace here, if we figure out what to do about `optional`
// being a core annotation name, and also a v1model match kind.
